...
> **Version:** v5.18.34-job-menu-hardening (PWA) / GAS Backend v5.18.16-write-flow-validation @605

> **Date:** 2026-05-14 | **Phase:** 75 (Jobs/Billing/Reports Focus + Smoke Cleanup Gate)

> **Status:** RECOVERED + HARDENED + FUNCTIONALLY AUDITED + CORE-AUDITED + FOCUS-AUDITED - repo restored from stable `db942bd`; PC/mobile login and menu runtime verified on live GitHub Pages; Sprint 74 Core System Audit score 100/100; Sprint 75 Jobs/Billing/Reports Focus Audit score 100/100; System Integrity Audit score 100/100; Menu Journey Audit score 100/100; Functional Menu Audit score 100/100; PC Reports loads the real reports module; PC Tax now has a real renderer; Billing legacy prototype shimmed out and PC Billing render no longer blanks when the production renderer paints directly into the DOM; Settings includes Runtime Self-Test, operations diagnostics, diagnostics export, and safe PWA cache repair; GAS redeployed at @605 with the gated smoke cleanup backend action; protected API smoke passes core menus including Vision/LINE read endpoints; Smoke Cleanup Planner now reports 0 candidates after deleting the 4 reviewed smoke records with archive-before-delete; Gemini secret readiness is currently false and must be restored in Script Properties before real Gemini Vision analysis is considered complete.
...
- Added `scripts/set_gas_secret.js` to set allowed GAS Script Properties through the protected admin API using environment variables only. No real Gemini/LINE secret value is stored in the repo. Gemini cannot be restored from this machine until a real key is provided through an environment variable such as `GEMINI_API_KEY`.
