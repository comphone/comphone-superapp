...
> **Version:** v5.18.11-menu-map (PWA) / GAS Backend v5.18.8-auth-session-store @544

> **Date:** 2026-05-06 | **Phase:** 50 (Mobile Menu Map Recovery)

> **Status:** STABLE - PC/mobile menu runtime restored; mobile More menu recovered; route/API index guard clean; protected API smoke and local UI audit pass
...
|---|---|---|
| App Version | `v5.18.11-menu-map` | `pwa/version_config.js` |
| Cache Version | `comphone-v5.18.11-menu-map-20260506_2315` | `pwa/version_config.js`, `pwa/sw.js` |
| Build Timestamp | `20260506_2315` | `pwa/version_config.js` |
...
### Phase 50 Current Review (2026-05-06)
...
- Service worker cache was bumped to `comphone-v5.18.11-menu-map-20260506_2315` to force browsers off stale assets.
...
|---|---:|---|---|
| Runtime config/version/cache | 94 | Strong | Central PWA config and service worker cache are aligned at `v5.18.11-menu-map`; auxiliary/archival pages still need cleanup. |
...
| Done | Route/API intelligence guard | Legacy `stock` quick action pointed to a missing mobile page and future drift had no fast detector. | Fixed in `v5.18.11-menu-map`; run `node scripts/build_code_index.js` before release. |
...
| Done | Mobile menu map recovery | Operational pages existed but were hidden from the reduced More menu, and several pages were redirected away before loading. | Fixed in `v5.18.11-menu-map`; full grouped menu restored and real pages can load directly. |
