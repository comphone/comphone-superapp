...
PC_V=$(grep -oP "[?&]v=\K[0-9.]+" pwa/dashboard_pc.html 2>/dev/null | head -1 || echo "X")
MOBILE_V=$(grep -oP "[?&]v=\K[0-9.]+" pwa/index.html 2>/dev/null | head -1 || echo "X")
if [ "$SW_V" = "$PC_V" ] && [ "$SW_V" = "$MOBILE_V" ]; then
  test_pass "Frontend versions match: $SW_V"
else
  test_fail "Version mismatch: sw.js=$SW_V vs dashboard=$PC_V vs mobile=$MOBILE_V"
fi
