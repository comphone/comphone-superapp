...
async function testDashboardAPI() {
  ...
  // Test 2: getKPIBundle
  try {
    const res = await callGas('getKPIBundle', {});
    TEST_FRAMEWORK.assertSuccess(res, 'getKPIBundle_success');
  } catch(e) {
    TEST_FRAMEWORK.record('getKPIBundle_success', false, e.message);
  }
...
async function testInventoryAPI() {
  ...
  try {
    ...
    TEST_FRAMEWORK.assertSuccess(res, 'getAllStock_success');
    if (res && res.stock) {
      TEST_FRAMEWORK.assertArray(res.stock, 'getAllStock_hasData');
    }
  } catch(e) {
    TEST_FRAMEWORK.record('getAllStock_success', false, e.message);
  }

  // Test 2: getLowStockAlert
  try {
    const res = await callGas('getLowStockAlert', {});
    TEST_FRAMEWORK.assertSuccess(res, 'getLowStockAlert_success');
  } catch(e) {
    TEST_FRAMEWORK.record('getLowStockAlert_success', false, e.message);
  }
...
async function testBillingAPI() {
  ...
  // Test 1: getBillingReport
  try {
    const today = new Date().toISOString().split('T')[0];
    const res = await callGas('getBillingReport', { date_from: today, date_to: today });
    if (res && (res.success === true || res.summary)) {
      TEST_FRAMEWORK.record('getBillingReport_success', true, 'API responded', res);
    } else {
      TEST_FRAMEWORK.record('getBillingReport_success', false, 'API did not return expected data', res);
    }
  } catch(e) {
    TEST_FRAMEWORK.record('getBillingReport_success', false, e.message);
  }
