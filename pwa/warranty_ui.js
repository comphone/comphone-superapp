...
async function showWarrantyDetail(warrantyId) {
  try {
    ...
    // ส่ง LINE (via sendLineMessage — sendWarrantyLine was removed)
    document.getElementById('btn-send-warranty-line')?.addEventListener('click', async () => {
      try {
        const msg = [
          `🛡️ แจ้งเตือนประกันสินค้า`,
          `ใบรับประกัน: ${w.warranty_id || w.job_id || '-'}`,
          `ลูกค้า: ${w.customer_name || '-'}`,
          `อุปกรณ์: ${w.device_model || '-'}`,
          `วันหมดอายุ: ${w.expiry_date || w.end_date || '-'}`,
          `สถานะ: ${w.status || '-'}`,
          w.terms ? `เงื่อนไข: ${w.terms}` : ''
        ].filter(Boolean).join('\n');
        await callAPI('sendLineMessage', { message: msg, room: 'TECHNICIAN' });
