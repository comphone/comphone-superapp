...
function doGet(e) {
  try {
    ...
    if (action === 'listbackups' || action === 'listBackups') {
      return jsonOutputV55_(listBackupsAPI(params));
    }
    if (action === 'checkbackuphealth' || action === 'checkBackupHealth') {
...
function _checkAuthGateV55_(action, payload, e) {
  ...
  var PUBLIC_ACTIONS = {
    ...
    // Phase 34 Backup & Recovery (TEMP for testing)
    'createbackup': 1, 'createBackup': 1,
    'listbackups': 1, 'listBackups': 1,
    'checkbackuphealth': 1, 'checkBackupHealth': 1,
