# -*- coding: utf-8 -*-
"""V326 - Patch existing Index.html: Dark Mode + Glassmorphism + Zero Bug"""
import os
...
# ============================================================
# PATCH 1: Replace CSS with Dark Mode Glassmorphism
# ============================================================

old_css_start = c.find('<style>')
old_css_end = c.find('</style>')

if old_css_start < 0 or old_css_end < 0:
    print("ERROR: No style tags found")
    exit(1)
...
# Find the old style block
old_style = c[old_css_start:old_css_end + 8]  # include </style>
c = c.replace(old_style, new_css)
print("[1] CSS replaced with Dark Mode Glassmorphism")

# ============================================================
# PATCH 2: Fix header version
# ============================================================
c = c.replace('Comphone V32', 'Comphone V326')
print("[2] Version updated to V326")

# ============================================================
# PATCH 3: Save for now (will do JS patches next)
# ============================================================
with open(path, "w", encoding="utf-8") as f:
    f.write(c)

print(f"\nInterim save: {len(c)} bytes")
...
r = subprocess.run(
    ["node", "-e",
     ...
     "try{new Function(m[1]);console.log('JS VALID')}catch(e){console.log('JS ERROR:',e.message)}"],
