...
// ============================================================
// STEP 1: Read current JS_Dashboard.html and split into 2 parts
// ============================================================
let jsFull = fs.readFileSync(D + '/JS_Dashboard.html', 'utf8');

// Remove <script> wrapper to get raw JS
let jsCode = jsFull.replace(/^<script>\s*\n?/, '').replace(/\s*<\/script>\s*$/, '');

// Find the midpoint - split at a clean function boundary
const functions = [];
const funcRe = /^function\s+(\w+)\s*\(/gm;
let m;
while ((m = funcRe.exec(jsCode)) !== null) {
  functions.push({ name: m[1], pos: m.index });
}
console.log('Total functions:', functions.length);
functions.forEach((f, i) => console.log(`  ${i+1}. ${f.name} (pos ${f.pos})`));

// Split at the middle function
const midIdx = Math.floor(functions.length / 2);
const splitPos = functions[midIdx].pos;

const part1Raw = jsCode.substring(0, splitPos).trimEnd();
const part2Raw = jsCode.substring(splitPos).trimStart();
...
// Write Part 1 & Part 2
fs.writeFileSync(D + '/JS_Part1.html', part1, 'utf8');
fs.writeFileSync(D + '/JS_Part2.html', part2, 'utf8');

console.log('\n=== Part 1 ===');
console.log('Size:', part1.length, 'bytes');
console.log('Functions:', (part1.match(/function \w+/g) || []).length);
console.log('First line:', part1.split('\n')[1]);

console.log('\n=== Part 2 ===');
console.log('Size:', part2.length, 'bytes');
console.log('Functions:', (part2.match(/function \w+/g) || []).length);
console.log('First line:', part2.split('\n')[1]);

// ============================================================
// STEP 2: Create proper Index.html with both includes
// ============================================================
...
<body>
<div class="hd"><h1>\ud83d\udcf1 Comphone V349</h1><div class="sub" id="clock">กําลังโหลด...</div></div>
...
console.log('\n=== Index.html ===');
console.log('Size:', indexHtml.length, 'bytes');
...
if (!dashGs.includes('function include(')) {
  const includeFn = `function include(filename) {\n  return HtmlService.createHtmlOutputFromFile(filename).getContent();\n}\n\n`;
  const pos = dashGs.indexOf('function doGet(');
  if (pos > 0) dashGs = dashGs.substring(0, pos) + includeFn + dashGs.substring(pos);
}

// Ensure createTemplateFromFile
...
console.log('Dashboard.gs updated ✅');
...
let pass = true;
const idxChecks = [
  ['Has <?!= JS_Part1', idx.includes("<?!= include('JS_Part1')")],
  ['Has <?!= JS_Part2', idx.includes("<?!= include('JS_Part2')")],
  ['Has .hd div', idx.includes('class="hd"')],
  ['Has #ld div', idx.includes('id="ld"')],
  ['Has #mo div', idx.includes('id="mo"')],
  ['Has .qab div', idx.includes('class="qab"')],
  ['Ends with </html>', idx.trim().endsWith('</html>')],
  ['Has <body>', idx.includes('<body>')],
];

for (const [n, ok] of idxChecks) {
  console.log(ok ? '✅' : '❌', n);
  if (!ok) pass = false;
}

// JS Part checks - combine both parts and validate
const p1Code = p1.replace(/^<script>\s*/, '').replace(/\s*<\/script>\s*$/, '');
const p2Code = p2.replace(/^<script>\s*/, '').replace(/\s*<\/script>\s*$/, '');
const combined = p1Code + '\n' + p2Code;

const jsChecks = [
  ['Part1 starts with <script>', p1.startsWith('<script>')],
  ['Part1 ends with </script>', p1.trim().endsWith('</script>')],
  ['Part2 starts with <script>', p2.startsWith('<script>')],
  ['Part2 ends with </script>', p2.trim().endsWith('</script>')],
];

for (const [n, ok] of jsChecks) {
  console.log(ok ? '✅' : '❌', n);
  if (!ok) pass = false;
}
...
// Count functions
const allFuncs = (combined.match(/function \w+/g) || []);
...
if (combined.includes('"cm()"')) { console.log('❌ cm() found'); pass = false; }
if (combined.includes('jtl">"')) { console.log('❌ jtl broken'); pass = false; }

// Dashboard.gs check
const dgs = fs.readFileSync(D + '/Dashboard.gs', 'utf8');
if (!dgs.includes('include(')) { console.log('❌ include() missing'); pass = false; }
else { console.log('✅ include() in Dashboard.gs'); }
if (!dgs.includes("createTemplateFromFile('Index')")) { console.log('❌ createTemplateFromFile missing'); pass = false; }
else { console.log('✅ createTemplateFromFile OK'); }

if (pass) {
  console.log('\n🎉 V349 ALL CHECKS PASSED!');
  console.log('Part1:', p1.length, 'bytes,', (p1.match(/function \w+/g)||[]).length, 'funcs');
  console.log('Part2:', p2.length, 'bytes,', (p2.match(/function \w+/g)||[]).length, 'funcs');
